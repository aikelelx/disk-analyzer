...
class DiskAnalyzerGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(top, text="Путь:").pack(side=tk.LEFT, padx=5)
        ttk.Entry(top, textvariable=self.current_path, width=60).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ttk.Button(top, text="Обзор", command=self.select_path).pack(side=tk.LEFT, padx=5)
        
        buttons = ttk.Frame(self.root, padding="5")
        buttons.pack(fill=tk.X)
        
        ttk.Button(buttons, text="Анализ", command=self.start_analyze).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Дубликаты", command=self.start_duplicates).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Старые файлы", command=self.start_old).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Пустые папки", command=self.start_empty).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Типы файлов", command=self.start_types).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Крупные файлы", command=self.start_large).pack(side=tk.LEFT, padx=3)
        ttk.Button(buttons, text="Выход", command=self.root.quit).pack(side=tk.LEFT, padx=3)
        ...
        self.text = tk.Text(frame, wrap=tk.WORD, yscrollcommand=scroll.set)
    ...
    def start_progress(self):
        ...
        self.status.config(text="Работает...")
